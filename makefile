...
COMPONENT_FILES	:= flex_counter.sv gf_Mod.sv gf_Mult.sv gf_Square.sv gf_Div.sv gf_Add.sv point_addition_doubling.sv

# Specify the name of the top level file (do not include the source folder in the name)
# NOTE: YOU WILL NEED TO SET THIS VARIABLE'S VALUE WHEN WORKING WITH HEIRARCHICAL DESIGNS
# AND THE AUTOMATED GRADING SYSTEM
TOP_LEVEL_FILE	:= point_multiplication.sv
...
mapped/$(TOP_MODULE).v: source/$(TOP_LEVEL_FILE) $(addprefix source/,$(COMPONENT_FILES))
	...
	@$(MAKE) --no-print-directory syn_mapped MAIN_FILE='$(TOP_LEVEL_FILE)' DEP_SUB_FILES='$(COMPONENT_FILES)' CLOCK_NAME='clk' CLOCK_PERIOD='4' > $(TOP_MODULE).log
...
report_area -hier >> reports/$(MOD_NAME).rep                                                  \n\
