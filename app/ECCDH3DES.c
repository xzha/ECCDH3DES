...
void read_SRAM(PCIE_HANDLE hPCIe, int fileSize, char * buffer)
{
    ...
    DWORD address;
	// Save output file
	FILE * fh = fopen("./output1.txt", "w");
	FILE * fh2 = fopen("./output2.txt", "w");
    ...
    for(x = 0; x < fileSize; x += 8)
    {
		...
		fprintf(fh, "%08x", read);
		fprintf(fh2, "%c", (read ) & 0xFF);
		fprintf(fh2, "%c", (read >> 8) & 0xFF);
		fprintf(fh2, "%c", (read >> 16) & 0xFF);
		fprintf(fh2, "%c", (read >> 24) & 0xFF);
...
void des(PCIE_HANDLE hPCIe, char encryption)
{
	...
	while(!des_done) 
	{
		...
		bPass = PCIE_Read32(hPCIe, pcie_bars[0], csr_registers(70), &des);
		printf("%08x ", des);
